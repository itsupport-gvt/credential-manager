...
function TenantForm({ initial = EMPTY, onSubmit, onCancel, submitting, isEdit }: {
  ...
  const ch = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => setForm(f => ({ ...f, [e.target.name]: e.target.value }))
    ...
    <form onSubmit={async e => { e.preventDefault(); await onSubmit(form) }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
        <div>
          ...
          <input style={{ ...inp, opacity: isEdit ? .6 : 1 }} name="tenant_code" value={form.tenant_code} onChange={ch} required disabled={isEdit} placeholder="e.g. ACME" />
        </div>
        <div>
          <label className="md-label">Tenant Name *</label>
          <input style={inp} name="tenant_name" value={form.tenant_name} onChange={ch} required placeholder="Acme Corporation" />
        </div>
        <div>
          <label className="md-label">Industry</label>
          <input style={inp} name="industry" value={form.industry} onChange={ch} />
        </div>
        <div>
          <label className="md-label">Primary Contact</label>
          <input style={inp} name="primary_contact" value={form.primary_contact} onChange={ch} />
        </div>
        <div>
          <label className="md-label">Contact Email</label>
          <input style={inp} type="email" name="contact_email" value={form.contact_email} onChange={ch} />
        </div>
        <div>
          <label className="md-label">Contact Phone</label>
          <input style={inp} type="tel" name="contact_phone" value={form.contact_phone} onChange={ch} />
        </div>
        <div>
          <label className="md-label">Account Manager</label>
          <input style={inp} name="account_manager" value={form.account_manager} onChange={ch} />
        </div>
        <div>
        ...
        <div>
          ...
          <input style={inp} type="date" name="contract_start" value={form.contract_start} onChange={ch} />
        </div>
        <div>
          <label className="md-label">Contract End</label>
          <input style={inp} type="date" name="contract_end" value={form.contract_end} onChange={ch} />
        </div>
        <div style={{ gridColumn: '1 / -1' }}>
          <label className="md-label">Notes</label>
          <textarea style={{ ...inp, resize: 'vertical' }} name="notes" value={form.notes} onChange={ch} rows={2} />
        </div>
      </div>
  ...
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        <div style={{ height: 36, width: 100, background: 'var(--surface-2)', borderRadius: 24 }} />
      </div>
      {Array.from({ length: 5 }).map((_, i) => <div key={i} style={{ height: 56, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, animation: 'pulse 1.5s infinite' }} />)}
    </div>
  )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {showAdd && (
        <div className="md-card animate-in" style={{ padding: 20 }}>
          <div style={{ fontWeight: 600, fontFamily: "'Google Sans', sans-serif", fontSize: 15, marginBottom: 16, color: 'var(--text-1)' }}>New Tenant</div>
      ...
      <div className="md-card" style={{ overflow: 'hidden' }}>
        {tenants.length === 0 ? (
          <div style={{ padding: '48px 0', textAlign: 'center', color: 'var(--text-3)', fontSize: 14 }}>
            <span className="icon" style={{ fontSize: 40, display: 'block', marginBottom: 10, opacity: .3 }}>business</span>
            No tenants yet. Add one above.
          </div>
        ) : tenants.map(t => (
          <div key={t.tenant_code} style={{ borderBottom: '1px solid var(--border)' }}>
            {/* Row */}
            <div className="md-row" style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 18px' }}
              onClick={() => { setExpanded(e => e === t.tenant_code ? null : t.tenant_code); setEditing(null) }}>
              <div style={{ width: 20, height: 20, borderRadius: 6, background: 'var(--primary-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <span className="icon icon-sm" style={{ color: 'var(--primary)', fontSize: 14 }}>business</span>
              </div>
              <div style={{ width: 100, flexShrink: 0 }}>
                <span style={{ fontFamily: 'monospace', fontWeight: 600, fontSize: 13, color: 'var(--text-1)' }}>{t.tenant_code}</span>
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 500, color: 'var(--text-1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.tenant_name}</div>
                {t.industry && <div style={{ fontSize: 12, color: 'var(--text-3)' }}>{t.industry}</div>}
              </div>
              <div style={{ flexShrink: 0, display: 'none' }} className="sm-contact">{t.primary_contact}</div>
              <span className={t.status === 'Active' ? 'badge-active' : 'badge-neutral'}>{t.status}</span>
              <div style={{ flexShrink: 0, fontSize: 13, color: 'var(--text-2)', textAlign: 'right' }}>
                <span style={{ fontWeight: 600, color: 'var(--text-1)' }}>{credCount(t.tenant_code)}</span> creds
              </div>
              <span className="icon icon-sm" style={{ color: 'var(--text-3)', flexShrink: 0 }}>{expanded === t.tenant_code ? 'expand_less' : 'expand_more'}</span>
            </div>
            ...
            {expanded === t.tenant_code && (
              <div style={{ padding: '16px 18px', background: 'var(--surface-2)', borderTop: '1px solid var(--border)' }}>
                {editing === t.tenant_code ? (
                  <TenantForm
                    initial={{ tenant_code: t.tenant_code, tenant_name: t.tenant_name, industry: t.industry, primary_contact: t.primary_contact, contact_email: t.contact_email, contact_phone: t.contact_phone, account_manager: t.account_manager, contract_start: t.contract_start, contract_end: t.contract_end, status: t.status, notes: t.notes }}
                  ...
                  <div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 14 }}>
                      {[['Tenant ID', t.tenant_id], ['Industry', t.industry], ['Contact Email', t.contact_email], ['Contact Phone', t.contact_phone], ['Account Manager', t.account_manager], ['Contract Start', t.contract_start], ['Contract End', t.contract_end], ['Status', t.status]].map(([label, value]) => (
                        <div key={label}>
                          <div style={{ fontSize: 11, color: 'var(--text-3)', fontWeight: 500, textTransform: 'uppercase', letterSpacing: .5, marginBottom: 2 }}>{label}</div>
                          <div style={{ fontSize: 13, color: 'var(--text-1)' }}>{value || '—'}</div>
                        </div>
                      ))}
                    </div>
                    {t.notes && <div style={{ marginBottom: 14 }}><div style={{ fontSize: 11, color: 'var(--text-3)', fontWeight: 500, textTransform: 'uppercase', letterSpacing: .5, marginBottom: 2 }}>Notes</div><div style={{ fontSize: 13, color: 'var(--text-1)' }}>{t.notes}</div></div>}
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button className="md-btn md-btn-tonal" onClick={() => setEditing(t.tenant_code)}>
                        <span className="icon icon-sm">edit</span>Edit
                      </button>
                      <button className="md-btn md-btn-danger" onClick={() => handleDelete(t.tenant_code)} disabled={deleting === t.tenant_code}>
