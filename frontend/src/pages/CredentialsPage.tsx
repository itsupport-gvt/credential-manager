...
export default function CredentialsPage() {
  ...
  const fetch = useCallback(() => {
    ...
    api.listCredentials({
      ...
      priority: filterPriority || undefined, page, page_size: PAGE_SIZE,
    })
      .then(setData)
  ...
  const hasFilters      = !!(query || filterTenant || filterCategory || filterStatus || filterPriority)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        <select value={filterPriority} onChange={e => { setFilterPriority(e.target.value); setPage(1) }} className="md-select" style={{ width: 'auto', minWidth: 130 }}>
          <option value="">All priorities</option>
          {PRIORITIES.map(p => <option key={p} value={p}>{p}</option>)}
        </select>
        {hasFilters && (
          <button
            onClick={() => { setQuery(''); setFilterTenant(''); setFilterCategory(''); setFilterStatus(''); setFilterPriority(''); setPage(1) }}
            ...
            <tbody>
              {loading ? (
                ...
                data.items.map((c: Credential) => (
                  <tr key={c.id} className="md-row"
                    style={{ borderBottom: '1px solid var(--border)' }}
                    onClick={() => navigate(`/credential/${c.credential_id}`)}>
                    <td style={{ padding: '14px 16px', fontFamily: 'monospace', fontSize: 11, color: 'var(--text-3)' }}>
                      {c.credential_id}
                    </td>
                    <td style={{ padding: '14px 16px' }}>
                      <div style={{ color: 'var(--text-1)', fontWeight: 500 }}>{c.service_name}</div>
                      {c.environment && <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2 }}>{c.environment}</div>}
                    </td>
                    <td style={{ padding: '14px 16px' }}>
                      <div style={{ color: 'var(--text-1)' }}>{c.tenant_name || c.tenant_code}</div>
                      {c.tenant_name && <div style={{ fontSize: 11, color: 'var(--text-3)', fontFamily: 'monospace', marginTop: 2 }}>{c.tenant_code}</div>}
                    </td>
                    <td style={{ padding: '14px 16px', color: 'var(--text-2)' }}>
                      {c.category}
                      {c.subcategory && <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2 }}>{c.subcategory}</div>}
                    </td>
                    <td style={{ padding: '14px 16px' }}>
                      <div
                        style={{ display: 'flex', alignItems: 'center', maxWidth: 200 }}
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      {c.password_expiry_date ? (
                        <span style={{
                          color: isExpired(c.password_expiry_date) ? 'var(--danger)'
                               : isExpiringSoon(c.password_expiry_date) ? 'var(--warn)'
                               : 'var(--text-2)',
                          fontWeight: (isExpired(c.password_expiry_date) || isExpiringSoon(c.password_expiry_date)) ? 500 : 400,
                        }}>
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      {c.mfa_enabled === 'Yes'
                        ? <span style={{ color: 'var(--success)', display: 'flex', alignItems: 'center', gap: 4, fontSize: 13 }}>
                            <span className="icon icon-sm">verified</span>Yes
                          </span>
                ...
                <tr>
                  <td colSpan={9} style={{ padding: '64px 0', textAlign: 'center' }}>
                    ...
                    <div style={{ color: 'var(--text-2)', fontSize: 14 }}>No credentials found</div>
                    {hasFilters && (
                      <button
                        onClick={() => { setQuery(''); setFilterTenant(''); setFilterCategory(''); setFilterStatus(''); setFilterPriority(''); setPage(1) }}
                        className="md-btn md-btn-text md-btn-sm"
        ...
        {data && totalPages > 1 && (
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button disabled={page <= 1} onClick={() => setPage(p => p - 1)} className="md-btn md-btn-outlined md-btn-sm">
                Previous
              </button>
              <button disabled={page >= totalPages} onClick={() => setPage(p => p + 1)} className="md-btn md-btn-outlined md-btn-sm">
                Next
              </button>
            </div>
          </div>
